...
`manual/AGENTS.md` and `rpa/AGENTS.md` already document their own module in depth (architecture, gotchas, working conventions) — read those directly rather than expecting duplicate detail here. `nl2sql/` has no AGENTS.md yet; its README covers commands and layout, summarized below.
...
No build/lint/test commands exist. See `manual/AGENTS.md` for the retrieval architecture (`bond_manual.md` chunking/scoring) and the reloader gotcha (editing `bond_manual.md` requires a process restart; template edits don't).
...
`rpa/PRD.md` is the source of truth for business rules (budget codes, shortfall-split logic, approval rules); `rpa/AGENTS.md` documents the chatbot state machine and data model in detail. When either the business rules or the chatbot behavior change, keep `PRD.md`, `AGENTS.md`, and `app.py` in sync — this module has previously drifted (a PDF-proof rule applied inconsistently between the two) and the fix was to correct the doc to match the code, not the reverse, after confirming intent. Don't resolve a doc/code mismatch unilaterally in either direction — ask which one is correct.

There is no automated test suite for `rpa/`; verification is manual, by running the Streamlit app and exercising the flows listed in `rpa/AGENTS.md`'s "Running / testing" section.
