...
from im_one_agent.cli import DEMO_QUESTIONS
...
# 로컬 통합 테스트 런처(launcher/app.py, http://127.0.0.1:7000)에서만 iframe 임베딩을 허용한다.
# 그 외 출처는 여전히 차단되므로 clickjacking 방어 목적은 유지된다.
ALLOWED_FRAME_ANCESTOR = "http://127.0.0.1:7000"
SECURITY_HEADERS: dict[str, str] = {
    "Content-Security-Policy": (
        ...
        "base-uri 'none'; "
        "form-action 'self'; "
        f"frame-ancestors {ALLOWED_FRAME_ANCESTOR}"
    ),
    "X-Content-Type-Options": "nosniff",
    "Referrer-Policy": "no-referrer",
    "Permissions-Policy": "camera=(), microphone=(), geolocation=()",
    # http.server has no default Cache-Control; without one, browsers apply
    # heuristic caching to security headers (CSP included), so a changed CSP
    # can silently keep enforcing a stale cached value. Disable caching outright.
...
class AgentWebHandler(SimpleHTTPRequestHandler):
    ...
    def do_GET(self) -> None:
        ...
        if parsed.path == "/api/health":
            include_sensitive = auth_mode() == "none" or is_authorized(self.headers)
            self.write_json(build_health_payload(self.db_path, include_sensitive=include_sensitive))
            return
        if parsed.path == "/api/demo-questions":
            self.write_json({"questions": DEMO_QUESTIONS})
            return
        if parsed.path in PROTECTED_GET_PATHS and not self.require_authorization():
            return
        if parsed.path == "/api/metrics":
            self.write_json(build_metrics_payload())
            return
        if parsed.path == "/api/feedback-summary":
            self.write_json(build_feedback_summary())
            return
        if parsed.path == "/api/audit-summary":
            self.write_json(build_audit_summary(Path(self.audit_path), db_path=self.db_path))
            return
        if parsed.path == "/api/catalog":
            query = parse_qs(parsed.query)
            role = normalize_user_role(query.get("role", ["branch_manager"])[0])
            self.write_json(build_catalog_payload(role=role))
            return
        if parsed.path == "/api/catalog-governance":
            query = parse_qs(parsed.query)
            role = normalize_user_role(query.get("role", ["branch_manager"])[0])
            self.write_json(build_catalog_governance_payload(role=role))
            return
        if parsed.path == "/api/evaluation-summary":
            query = parse_qs(parsed.query)
            role = normalize_user_role(query.get("role", ["sales_planning"])[0])
            branch_id = normalize_branch_id(query.get("branchId", [DEFAULT_BRANCH_ID])[0])
            self.write_json(build_evaluation_case_summary(role=role, branch_id=branch_id))
            return
        if parsed.path == "/api/readiness":
            query = parse_qs(parsed.query)
            try:
                profile = normalize_readiness_profile(query.get("profile", [""])[0])
                live_checks = normalize_readiness_live_checks(query.get("live", [""])[0])
            except PayloadValidationError as exc:
                self.write_json({"error": str(exc)}, status=HTTPStatus.BAD_REQUEST)
                return
            self.write_json(build_readiness_payload(self.db_path, profile=profile, live_checks=live_checks))
            return
        if parsed.path == "/api/verified-questions":
            query = parse_qs(parsed.query)
            role = normalize_user_role(query.get("role", ["sales_planning"])[0])
            branch_id = normalize_branch_id(query.get("branchId", [DEFAULT_BRANCH_ID])[0])
            self.write_json(build_verified_question_manifest(role=role, branch_id=branch_id))
            return
    ...
    def do_POST(self) -> None:
        parsed = urlparse(self.path)
        if parsed.path not in PROTECTED_POST_PATHS:
        ...
        try:
            ...
            query_payload = parse_query_payload(payload)
            identity = resolve_request_identity(self.headers, query_payload.role, query_payload.branch_id)
        except PayloadTooLargeError as exc:
            increment_metric("errors_total")
            self.write_json({"error": str(exc)}, status=HTTPStatus.REQUEST_ENTITY_TOO_LARGE)
            return
        except PayloadValidationError as exc:
            increment_metric("errors_total")
            self.write_json({"error": str(exc)}, status=HTTPStatus.BAD_REQUEST)
            ...
            increment_metric("errors_total")
            self.write_json({"error": "요청 형식이 올바르지 않습니다."}, status=HTTPStatus.BAD_REQUEST)
            return
        ...
        try:
            response = run_agent(
                ...
                identity.role,
                identity.branch_id,
                query_payload.conversation_context,
                self.db_path,
                self.audit_path,
                user_id=identity.user_id,
                request_auth_mode=identity.auth_mode,
            )
        except Exception as exc:  # pragma: no cover - keeps the local demo server resilient
            increment_metric("errors_total")
            self.write_json({"error": f"에이전트 실행 중 오류가 발생했습니다: {exc}"}, status=HTTPStatus.INTERNAL_SERVER_ERROR)
            return

        increment_metric("queries_total")
        validation = response.get("validation", {})
        if validation.get("allowed"):
            increment_metric("queries_allowed_total")
        else:
            increment_metric("queries_blocked_total")
            issues = " ".join(validation.get("issues", []))
            if "LLM SQL 생성 실패" in issues:
                increment_metric("llm_generation_failures_total")

        response["sessionId"] = query_payload.session_id
        store_session_result(query_payload.session_id, response)
    ...
    def handle_export(self) -> None:
        try:
            payload = read_json_body(self.headers, self.rfile)
            session_id = normalize_session_id(payload.get("sessionId"))
            export_type = normalize_export_type(payload.get("exportType", "csv"))
            payload_role = str(payload.get("role", "branch_manager")).strip() or "branch_manager"
            payload_branch_id = int(payload.get("branchId", 1))
            identity = resolve_request_identity(self.headers, payload_role, payload_branch_id)
        except PayloadTooLargeError as exc:
            increment_metric("errors_total")
            self.write_json({"error": str(exc)}, status=HTTPStatus.REQUEST_ENTITY_TOO_LARGE)
            return
        ...
        if not result:
            increment_metric("errors_total")
            self.write_json({"error": "내보낼 실행 결과가 없습니다."}, status=HTTPStatus.NOT_FOUND)
            return
        if not session_result_accessible(result, identity):
            increment_metric("errors_total")
            self.write_json({"error": "해당 세션 결과에 접근할 권한이 없습니다."}, status=HTTPStatus.FORBIDDEN)
            return

        increment_metric("exports_total")
        if export_type == "report":
            self.write_text(
                build_report_draft(result),
                content_type="text/markdown; charset=utf-8",
                filename="im-one-report.md",
            )
            return

        self.write_text(
            build_csv_document(result.get("columns", []), result.get("rows", [])),
            content_type="text/csv; charset=utf-8",
            filename="im-one-results.csv",
        )
    ...
    def handle_feedback(self) -> None:
        ...
        if not normalize_feedback_rating(payload.get("rating")) and not normalize_feedback_comment(payload.get("comment")):
            increment_metric("errors_total")
            self.write_json({"error": "피드백 rating 또는 comment가 필요합니다."}, status=HTTPStatus.BAD_REQUEST)
            return

        session_result, error_status, error_message = session_result_for_feedback(session_id, identity)
        if error_status is not None:
            increment_metric("errors_total")
            self.write_json({"error": error_message}, status=error_status)
            return

        event = build_feedback_event(payload, identity, session_result)
        append_feedback_event(event)
        increment_metric("feedback_total")
        self.write_json({"ok": True, "feedback": event})

    def require_authorization(self) -> bool:
        if is_authorized(self.headers):
            return True
        increment_metric("errors_total")
        self.write_json({"error": "인증이 필요합니다."}, status=HTTPStatus.UNAUTHORIZED)
        return False
    ...
    def write_text(
        ...
        encoded = payload.encode("utf-8-sig" if content_type.startswith("text/csv") else "utf-8")
        self.send_response(status)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the iM One Agent web UI.")
...
def main() -> None:
    args = parse_args()
    AgentWebHandler.db_path = args.db_path
    AgentWebHandler.audit_path = args.audit_path

    server = ThreadingHTTPServer((args.host, args.port), AgentWebHandler)
    print(f"iM One Agent UI: http://{args.host}:{args.port}")
    try:
