...
def test_demo_documents_reflect_current_poc_capabilities() -> None:
    ...
    for required in (
        ...
        "clarification chip",
        "100+ verified question variants",
        "IM_ONE_QUERY_TIMEOUT_MS",
        "--profile pilot",
    ):
...
def test_preflight_blocks_external_or_incomplete_static_assets(tmp_path) -> None:
    ...
    (static_dir / "app.js").write_text("window.app = {};\n", encoding="utf-8")
    (static_dir / "styles.css").write_text(":root { color-scheme: dark; }\n", encoding="utf-8")
    (static_dir / "icons.js").write_text(
        "window.lucide = { createIcons() {} }; const ICONS = { check: '' };\n",
        encoding="utf-8",
    )
    (static_dir / "layout.js").write_text("window.calculateResultGridHeight = () => 140;\n", encoding="utf-8")

    check = check_static_ui_assets(static_dir)

    assert not check.passed
    assert check.required
    assert "index.html missing local reference: href=\"/styles.css\"" in check.detail
    assert "index.html missing status selector: id=\"generationLabel\"" in check.detail
    assert "index.html missing PRD layout contract: id=\"homeButton\"" in check.detail
    assert "app.js missing status renderer: data.generationEngine" in check.detail
    assert "styles.css missing PRD responsive/layout contract: @media (max-width: 820px)" in check.detail
    assert "index.html references external script or stylesheet URLs." in check.detail
    assert "icons.js missing icons: missing-icon" in check.detail
...
def test_preflight_reports_embedding_configuration(tmp_path, monkeypatch) -> None:
    db_path = tmp_path / "im_one_demo.sqlite"
    monkeypatch.setenv("OPENAI_API_KEY", "test-key")
    ...
    assert "text-embedding-3-small" in by_name["embedding_configuration"].detail
