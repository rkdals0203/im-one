...
def test_static_ui_exposes_role_policy_controls() -> None:
    html = Path("src/im_one_agent/static/index.html").read_text(encoding="utf-8")
    script = Path("src/im_one_agent/static/app.js").read_text(encoding="utf-8")
    web_source = Path("src/im_one_agent/web.py").read_text(encoding="utf-8")

    assert 'id="roleSelect"' in html
    assert 'value="branch_manager"' in html
    assert 'value="sales_planning"' in html
    assert 'value="compliance"' in html
    assert 'id="branchScopeInput"' in html
    assert "role: roleSelect.value" in script
    assert "branchId: currentBranchId()" in script
    assert "renderSchemaList(data.tables || [])" in script
    assert 'id="homeSampleList"' in html
    assert 'id="homeInput"' in html
    assert 'required' in html
    assert 'aria-required="true"' in html
    assert 'id="chatInput"' in html
    assert f'id="homeInput"\n                rows="3"\n                autocomplete="off"\n                maxlength="{MAX_QUESTION_LENGTH}"' in html
    assert f'id="questionInput" name="question" rows="2" autocomplete="off" maxlength="{MAX_QUESTION_LENGTH}"' in html
    assert f'id="chatInput" type="text" placeholder="Ask your data a question..." maxlength="{MAX_QUESTION_LENGTH}"' in html
    assert f"const questionMaxLength = {MAX_QUESTION_LENGTH}" in script
    assert "homeSampleList.innerHTML" in script
    assert "home-sample-button" in script
    assert "validateQuestionInput(input)" in script
    assert "validateQuestionInput(homeInput)" in script
    assert "validateQuestionInput(chatInput)" in script
    assert "reportValidity()" in script
    assert 'id="feedbackForm"' in html
    assert 'data-feedback-rating="up"' in html
    assert 'data-feedback-rating="down"' in html
    assert 'id="feedbackCategory"' in html
    assert 'id="feedbackComment"' in html
    assert f'id="feedbackComment" type="text" placeholder="Add note..." maxlength="{MAX_FEEDBACK_COMMENT_LENGTH}"' in html
    assert 'id="metricList"' in html
    assert "<summary>investment_reviews</summary>" in html
    assert "<summary>branch_targets</summary>" in html
    assert "<summary>investment_review_status</summary>" in html
    assert "<summary>new_accounts_vs_target</summary>" in html
    assert 'id="clarificationList"' in html
    assert 'id="catalogButton"' in html
    assert 'id="catalogView"' in html
    assert 'id="catalogRoleSelect"' in html
    assert 'id="catalogManagementSummary"' in html
    assert 'id="catalogMetricManagementList"' in html
    assert 'id="catalogTableManagementList"' in html
    assert 'id="catalogRoleCoverageList"' in html
    assert 'id="catalogIssueManagementList"' in html
    assert 'id="monitorButton"' in html
    assert 'id="monitorView"' in html
    assert 'id="runtimeMetrics"' in html
    assert 'id="auditSummaryList"' in html
    assert 'id="feedbackSummaryList"' in html
    assert 'id="verifiedSummary"' in html
    assert 'id="catalogGovernanceSummary"' in html
    assert 'id="evaluationSummary"' in html
    assert 'id="readinessSummary"' in html
    assert 'id="readinessActionList"' in html
    assert "local_required_failed" in html
    assert "poc_required_failed" in html
    assert "pilot_required_failed" in html
    assert "poc_gate" in script
    assert "pilot_gate" in script
    assert "poc_eval_gate" in script
    assert "pilot_eval_gate" in script
    assert "readiness_gate?.failures" in script
    assert "live_checks_not_run" in script
    assert "prd_evaluation_gate?.failures" in script
    assert "prd_evaluation_gate?.coverage_gate?.failures" in script
    assert "prd_evaluation_evidence_not_run" in script
    assert "prd_evaluation_coverage_failed" in script
    assert '"/api/feedback"' in script
    assert "/api/catalog?role=${" in script
    assert 'fetchMonitorJson("/api/metrics")' in script
    assert 'fetchMonitorJson("/api/feedback-summary")' in script
    assert 'fetchMonitorJson("/api/audit-summary")' in script
    assert "fetchMonitorJson(`/api/catalog?role=${" in script
    assert "fetchMonitorJson(`/api/catalog-governance?role=${" in script
    assert "fetchMonitorJson(`/api/evaluation-summary?role=${" in script
    assert 'fetchMonitorJson("/api/readiness")' in script
    assert 'fetchMonitorJson("/api/readiness?profile=poc")' in script
    assert 'fetchMonitorJson("/api/readiness?profile=pilot")' in script
    assert "fetchMonitorJson(`/api/verified-questions?role=${" in script
    assert "renderMetricList(data.metrics || [])" in script
    assert "renderMonitorStats(runtimeMetrics" in script
    assert "renderMonitorStats(catalogGovernanceSummary" in script
    assert "renderMonitorStats(catalogManagementSummary" in script
    assert "renderCatalogMetricManagement(catalogMetricManagementList" in script
    assert "renderCatalogTableManagement(catalogTableManagementList" in script
    assert "renderCatalogRoleCoverage(catalogRoleCoverageList" in script
    assert "renderCatalogIssueManagement(catalogIssueManagementList" in script
    assert "renderMonitorStats(evaluationSummary" in script
    assert "renderMonitorStats(readinessSummary" in script
    assert "renderReadinessActions(readinessActionList" in script
    assert "renderCounterSummary(" in script
    assert "renderBacklogSummary(feedbackSummaryList, feedback.semantic_backlog" in script
    assert "renderClarificationOptions(data.clarificationOptions || [])" in script
    assert ".clarification-chip" in script
    assert "selectedFeedbackRating" in script
    assert "data.retryGuidance" in script
    assert '"retryGuidance": retry_guidance_for(validation)' in web_source
    assert '"/api/verified-questions"' in web_source
    assert '"/api/feedback-summary"' in web_source
    assert '"/api/audit-summary"' in web_source
    assert '"/api/catalog"' in web_source
    assert '"/api/catalog-governance"' in web_source
    assert '"/api/evaluation-summary"' in web_source
    assert '"/api/readiness"' in web_source
    assert "build_verified_question_manifest" in web_source


def test_static_ui_covers_home_navigation_and_responsive_table_layout() -> None:
    html = Path("src/im_one_agent/static/index.html").read_text(encoding="utf-8")
    script = Path("src/im_one_agent/static/app.js").read_text(encoding="utf-8")
    styles = Path("src/im_one_agent/static/styles.css").read_text(encoding="utf-8")

    assert 'id="homeButton"' in html
    assert 'aria-label="Home"' in html
    assert 'id="workspaceButton"' in html
    assert 'id="monitorButton"' in html
    assert "homeButton.addEventListener(\"click\", showHome)" in script
    assert "workspaceButton.addEventListener(\"click\", showWorkspace)" in script
    assert "monitorButton.addEventListener(\"click\", showMonitor)" in script
    assert "refreshMonitorButton.addEventListener(\"click\", loadMonitoring)" in script
    assert "showHome();" in script
    assert "showWorkspace();" in script
    assert 'src="/layout.js"' in html
    assert "window.addEventListener(\"resize\", syncResultGridHeight)" in script
    assert "dataPane.style.setProperty(\"--result-grid-height\"" in script
    assert "window.calculateResultGridHeight({" in script
    assert "tableScrollHeight: table.scrollHeight" in script
    assert "dataPaneHeight: dataPane.clientHeight" in script
    assert "grid-template-rows: auto auto minmax(132px, 1fr) var(--result-grid-height) auto;" in styles
    assert ".data-grid-wrap" in styles
    ...
    assert ".data-pane {\n    min-height: 720px;" in styles


def test_result_grid_height_helper_preserves_sql_space_and_bottom_table_fit() -> None:
    node_path = shutil.which("node")
    if node_path is None:
        pytest.skip("node is required to execute static layout helper")

    script = """
const { calculateResultGridHeight } = require("./src/im_one_agent/static/layout.js");
const tallTable = calculateResultGridHeight({
  dataPaneHeight: 760,
  tableTopbarHeight: 44,
  queryComposerHeight: 82,
  gridFooterHeight: 34,
  tableScrollHeight: 1200
});
const smallViewport = calculateResultGridHeight({
  dataPaneHeight: 320,
  tableTopbarHeight: 44,
  queryComposerHeight: 82,
  gridFooterHeight: 34,
  tableScrollHeight: 900
});
const shortTable = calculateResultGridHeight({
  dataPaneHeight: 760,
  tableTopbarHeight: 44,
  queryComposerHeight: 82,
  gridFooterHeight: 34,
  tableScrollHeight: 96
});
if (tallTable !== 450) throw new Error(`expected tall table height 450, got ${tallTable}`);
if (smallViewport !== 140) throw new Error(`expected minimum small viewport height 140, got ${smallViewport}`);
if (shortTable !== 96) throw new Error(`expected short table height 96, got ${shortTable}`);
"""
    subprocess.run([node_path, "-e", script], check=True, cwd=Path.cwd())


def test_static_ui_uses_local_icons_without_external_cdn() -> None:
    html = Path("src/im_one_agent/static/index.html").read_text(encoding="utf-8")
    icons = Path("src/im_one_agent/static/icons.js").read_text(encoding="utf-8")
    icon_names = set(re.findall(r'data-lucide="([^"]+)"', html))

    assert 'src="/icons.js"' in html
    assert "unpkg.com" not in html
    assert "https://" not in html
    assert "window.lucide" in icons
    assert "createIcons()" in icons
    assert icon_names
    for icon_name in icon_names:
        assert f'"{icon_name}"' in icons or f"{icon_name}:" in icons


def test_web_security_headers_restrict_static_app_to_self() -> None:
    ...
    # frame-ancestors is scoped to the local test launcher's origin only —
    # all other origins remain blocked (see ALLOWED_FRAME_ANCESTOR in web.py).
    assert "frame-ancestors http://127.0.0.1:7000" in csp
...
class FakeWebHandler(AgentWebHandler):
    def __init__(self, path: str, headers: dict[str, str] | None = None) -> None:
        ...
        self.db_path = ":memory:"
        self.audit_path = "/private/tmp/im-one-test-audit.jsonl"

    def require_authorization(self) -> bool:
        self.authorization_checked = True
        return super().require_authorization()

    def write_json(self, payload: dict[str, object], status: HTTPStatus = HTTPStatus.OK) -> None:
        self.responses.append((status, payload))


def test_handler_auth_policy_lists_public_and_protected_paths() -> None:
    assert PUBLIC_GET_PATHS == {"/api/health", "/api/demo-questions"}
    assert PROTECTED_GET_PATHS == {
        "/api/metrics",
    ...
    assert PROTECTED_POST_PATHS == {"/api/query", "/api/export", "/api/feedback"}


def test_protected_get_paths_require_authorization(monkeypatch) -> None:
    monkeypatch.setenv("IM_ONE_API_TOKEN", "test-token")

    for path in sorted(PROTECTED_GET_PATHS):
        handler = FakeWebHandler(path)
        handler.do_GET()

        assert handler.authorization_checked
        assert handler.responses == [(HTTPStatus.UNAUTHORIZED, {"error": "인증이 필요합니다."})]


def test_public_get_paths_do_not_require_authorization(monkeypatch, tmp_path) -> None:
    monkeypatch.setenv("IM_ONE_API_TOKEN", "test-token")

    health_handler = FakeWebHandler("/api/health")
    health_handler.db_path = str(tmp_path / "public-health.sqlite")
    health_handler.do_GET()
    demo_handler = FakeWebHandler("/api/demo-questions")
    demo_handler.do_GET()

    assert not health_handler.authorization_checked
    assert not demo_handler.authorization_checked
    assert health_handler.responses[0][0] == HTTPStatus.OK
    assert demo_handler.responses[0][0] == HTTPStatus.OK
    health_payload = health_handler.responses[0][1]
    assert "path" not in health_payload["database"]
    assert "model" not in health_payload["llm"]
    assert "base_url" not in health_payload["llm"]
    assert "model" not in health_payload["embedding"]
    assert "base_url" not in health_payload["embedding"]


def test_authorized_health_request_can_include_runtime_details(monkeypatch, tmp_path) -> None:
    monkeypatch.setenv("IM_ONE_API_TOKEN", "test-token")
    monkeypatch.setenv("IM_ONE_LLM_MODEL", "gpt-approved")
    monkeypatch.setenv("IM_ONE_LLM_BASE_URL", "https://llm.internal.example/v1")

    handler = FakeWebHandler("/api/health", headers={"Authorization": "Bearer test-token"})
    handler.db_path = str(tmp_path / "authorized-health.sqlite")
    handler.do_GET()

    status, payload = handler.responses[0]
    assert status == HTTPStatus.OK
    assert payload["database"]["path"] == str(tmp_path / "authorized-health.sqlite")
    assert payload["llm"]["model"] == "gpt-approved"
    assert payload["llm"]["base_url"] == "https://llm.internal.example/v1"


def test_protected_post_paths_require_authorization_before_body_parsing(monkeypatch) -> None:
    monkeypatch.setenv("IM_ONE_API_TOKEN", "test-token")

    for path in sorted(PROTECTED_POST_PATHS):
        handler = FakeWebHandler(path)
        handler.do_POST()
